...
app.get("/list-files", (req, res) => {
  const domainFilter = req.query.domain; // ?domain=example.com

  // Build base SQL and optional WHERE condition
  const baseQuery = `
    SELECT id, domain_name 
    FROM domains 
    ${domainFilter ? "WHERE domain_name = ?" : ""} 
    ORDER BY domain_name ASC
  `;

  const queryParams = domainFilter ? [domainFilter] : [];

  db.query(baseQuery, queryParams, async (err, rows) => {
    if (err) {
    ...
    const results = await Promise.allSettled(
      domains.map((domainRow) => {
        return new Promise((resolve, reject) => {
          db.query(
            `SELECT id, file_name, uploaded_at, content_hash, uploaded_hash
             FROM crawl_outputs
             WHERE domain_id = ?
            ...
            (err2, files) => {
              ...
              const fileList = files.map((file) => {
                const status = file.uploaded_hash
                  ? file.uploaded_hash === file.content_hash
                    ? "indexed"
                    : "edited"
                  : "not_indexed";
...
app.put("/file/:id", express.json(), (req, res) => {
  ...
  try {
    ...
    // Step 1: Check current content hash in DB
    db.query(
      `SELECT content_hash FROM crawl_outputs WHERE id = ?`,
      [fileId],
      (err, rows) => {
        if (err) {
          console.error("❌ Failed to fetch file hash:", err);
        ...
        // Step 2: If hash matches, no changes
        if (oldHash === newHash) {
          return res.json({
            success: true,
            changed: false,
