$(document).ready(function () {
  ...
  let firstPollDone = false; // declare outside function, global in scope

  $("#index-btn").hide(); // hide it immediately on page load

  function pollPerDomainCrawlStatus() {
    fetch("../../api/crawl_status.php")
      .then((res) => res.json())
      .then((data) => {
        let showIndexBtn = true;
        ...
        // ✅ Only show the index-btn after first poll (regardless of status)
        if (!firstPollDone) {
          $("#index-btn").show();
          firstPollDone = true;
        }

        // Optional: disable the button if crawling is in progress
        if (showIndexBtn) {
          $("#index-btn").prop("disabled", false).removeClass("disabled");
        } else {
          $("#index-btn").prop("disabled", true).addClass("disabled");
        }
      })
      .catch((err) => {
  ...
  $.get("../../api/domain-files.php", function (response) {
    const flatFiles = response.files || [];

    // ✅ Group files by domain_name
    const groupedByDomain = {};
    flatFiles.forEach((file) => {
      const domain = file.domain_name;
      if (!groupedByDomain[domain]) {
        groupedByDomain[domain] = [];
      }

      // Normalize file name property for display
      file.name = file.file_name || file.name || "Unnamed";
      groupedByDomain[domain].push(file);
    });

    // Clear loader
    $(".kb-container").empty();

    // ✅ Iterate over grouped domains
    Object.entries(groupedByDomain).forEach(([domain, files]) => {
